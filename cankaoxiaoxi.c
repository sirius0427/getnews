...
//http://www.cankaoxiaoxi.com/finance/zgcj/ 参考消息
int zgcj( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"inner\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "参考消息中国财经" );
			strcpy( s[n].sourcename, "参考消息中国财经" );
