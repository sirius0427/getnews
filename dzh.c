...
int hotnews( char *input_html, struct NewsInfo *s )
{
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.gw.com.cn/%s", tmpstr );
			...
			strcpy( s[n].articleauthor, "大智慧热点新闻" );
			strcpy( s[n].sourcename, "大智慧热点新闻" );
