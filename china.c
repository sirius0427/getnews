...
//http://finance.china.com.cn/news/live.shtml 中国财经滚动新闻
int finance( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"news_list\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		{
			input_html = input_html + iStart ;
			iEnd = indexOf( input_html, "</li>" );
			strncpy( tmp_html, input_html, iEnd);
			iStart=indexOf( tmp_html,"[<a" );
			if( iStart != -1 )
			{
			...
			strcpy( s[n].articleauthor, "中国财经滚动新闻" );
			strcpy( s[n].sourcename, "中国财经滚动新闻" );
