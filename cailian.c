...
//http://www.cailianpress.com/ 财联社
int cailian( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"entries\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"加载更多内容");
	input_html[iEnd] = '\0';

	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		memset(tmpstr,0x00,sizeof(tmpstr)); //初始化
		iStart=indexOf(input_html,"\"content ");
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.cailianpress.com/" );  //sourceaddr 

			replace( tmpstr, "\r", "" );
			replace( tmpstr, "\n", "" );
			replace( tmpstr, "\t", "" );
			replace( tmpstr, " ", "" );
			if( strlen( tmpstr ) < 801 )
			{
				sprintf( s[n].title, "%s", tmpstr );
			}
			else
			{
				strncpy( s[n].title, tmpstr, 801 );
			}
			//printf( "[%s]\n", tmpstr );
			...
			strcpy( s[n].articleauthor, "财联社" );
			strcpy( s[n].sourcename, "财联社" );
