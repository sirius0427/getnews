...
//http://wallstreetcn.com/news 华尔街见闻 
int news( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"news-item__main\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			if( strncmp( tmpstr, "http", 4 ) == 0 )
			{
				sprintf( s[n].sourceaddr, "%s", tmpstr );  //sourceaddr
			}
			else
				sprintf( s[n].sourceaddr, "https://wallstreetcn.com%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "华尔街见闻" );
			strcpy( s[n].sourcename, "华尔街见闻" );
