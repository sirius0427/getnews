...
//http://search.ccgp.gov.cn/bxsearch?searchtype=1&page_index=2&bidSort=0&buyerName=&projectId=&pinMu=0&bidType=7&dbselect=bidx&kw=&timeType=2&displayZone=&zoneId=&pppStatus=0&agentName=
//中国政府采购网标讯
int zhongbiao( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"vT-srch-result-list-bid\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "中国政府采购网" );
			strcpy( s[n].sourcename, "中国政府采购网" );
