...
//http://finance.huanqiu.com/roll/ 环球财经滚动
int roll( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"listPicBox\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			if( !strcmp( tmpstr, "" ))
			{
				iStart = indexOf( input_html, "<h5>") + 4; 
				input_html = input_html + iStart;
				iEnd=indexOf( input_html, "<em>");
				if( iEnd >= 200 )
					strncpy( tmpstr, input_html, 201 );
				else
					strncpy( tmpstr, input_html, iEnd );
			}
			...
			strcpy( s[n].articleauthor, "环球财经滚动" );
			strcpy( s[n].sourcename, "环球财经滚动" );
