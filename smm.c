...
int news( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"news\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"tetsteete\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://news.smm.cn%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "上海有色金属网" );
			strcpy( s[n].sourcename, "上海有色金属网" );
