...
//http://www.farmer.com.cn/xwpd/cjxw/ 中国农业新闻网 
int cjxw( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"yui3-g list-list-li\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"pages\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.farmer.com.cn/xwpd/cjxw%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "中国农业新闻网财经" );
			strcpy( s[n].sourcename, "中国农业新闻网财经" );
