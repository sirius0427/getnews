...
//http://www.eeo.com.cn/ 经济观察网
int eeo( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"index_list_1\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"xd-jzzl\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "经济观察网" );
			strcpy( s[n].sourcename, "经济观察网" );
