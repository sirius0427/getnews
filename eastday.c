...
//http://news.eastday.com/eastday/13news/auto/news/finance/index_K47.html 东方网财经滚动
int finance( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"newlist2\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "东方网财经滚动" );
			strcpy( s[n].sourcename, "东方网财经滚动" );
