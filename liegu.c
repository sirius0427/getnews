...
int liegu( char *input_html, struct NewsInfo *s )
{
	...
	for( n=0;n<100; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			if( tmpstr[0] == 'D' )
			{
				sprintf( s[n].sourceaddr, "http://www.lieguwang.com/%s", tmpstr );
			}
			else
			...
			strcpy( s[n].articleauthor, "猎股资讯" );
			strcpy( s[n].sourcename, "猎股资讯" );
