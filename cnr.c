...
//http://finance.cnr.cn/gundong/ 央广网财经滚动
int finance( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"articleList\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		iStart=indexOf(input_html,"<strong>");
		if( iStart != -1)
		{
			memset(tmpstr,0x00,sizeof(tmpstr)); //初始化
			...
			iStart = indexOf( input_html, "blank>") + 6; 
			input_html = input_html + iStart;
			iEnd=indexOf( input_html, "<");
			strncpy( tmpstr, input_html, iEnd );
			...
			strcpy( s[n].articleauthor, "央广网财经滚动" );
			strcpy( s[n].sourcename, "央广网财经滚动" );
