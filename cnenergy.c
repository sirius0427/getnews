...
//http://www.cnenergy.org/ 中国能源网
int news( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"main4_b1 main4_b1_2\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"main4_b1 main4_b1_3\"");
	input_html[iEnd] = '\0';

	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		memset(tmpstr,0x00,sizeof(tmpstr)); //初始化
		iStart=indexOf(input_html,"if(");
		input_html = input_html + iStart;
		if( iStart != -1)
		{
			memset(tmpstr,0x00,sizeof(tmpstr)); //初始化
			...
			sprintf( s[n].sourceaddr, "http://www.cnenergy.org%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "中国能源网" );
			strcpy( s[n].sourcename, "中国能源网" );
