...
//http://finance.ynet.com/2.1.0/84168.html 北青网财经滚动新闻
int finance( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"hc3 vb\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"fr\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "北青网财经滚动新闻" );
			strcpy( s[n].sourcename, "北青网财经滚动新闻" );
