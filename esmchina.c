...
//http://www.esmchina.com/ 国际电子商情网
int esmchina( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"class=\"caption\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"section");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.esmchina.com/%s", tmpstr );  //sourceaddr 
			...
			strcpy( s[n].articleauthor, "国际电子商情网" );
			strcpy( s[n].sourcename, "国际电子商情网" );
