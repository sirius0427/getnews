...
int govcnzhengce( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"list_1");
	...
	for( n=0;n<100; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			if( tmpstr[0] == '/' )
			{
				sprintf( s[n].sourceaddr, "http://www.gov.cn%s", tmpstr );
			}
			else
			...
			strcpy( s[n].articleauthor, "中央人民政府政策" );
			strcpy( s[n].sourcename, "中央人民政府政策" );
