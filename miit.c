...
//http://www.miit.gov.cn/n1146295/n1652858/index.html 工信部政策文件
int zhengce( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"clist_con\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			memset(tmpstr,0x00,sizeof(tmpstr)); //初始化
			iStart=indexOf(input_html,"href=");
			input_html = input_html + iStart + 5;
			iEnd=indexOf( input_html, " ");
			strncpy( tmpstr, input_html, iEnd);
			sprintf( s[n].sourceaddr, "http://www.miit.gov.cn/n1146295/n1652858/%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "工信部政策文件" );
			strcpy( s[n].sourcename, "工信部政策文件" );
