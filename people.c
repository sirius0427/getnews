...
//http://finance.people.com.cn/GB/70846/index.html 人民网财经新闻
int finance( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"list_14 list_ej\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"p1_right ej_right\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://finance.people.com.cn%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "人民网财经新闻" );
			strcpy( s[n].sourcename, "人民网财经新闻" );
