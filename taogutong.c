...
int taogutong( char *input_html, struct NewsInfo *s )
{
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		iStart=indexOf(input_html,"NewsID");
		if( iStart != -1)
		{
			iStart=indexOf(input_html,"NewsID");
			...
			sprintf( s[n].sourceaddr, "http://www.taogutong.com/shownews.aspx?id=%s", tmpstr );

			memset( tmpstr, 0x00, sizeof( tmpstr ));
			iStart = indexOf( input_html, "NewsTitle") + 12;
			...
			memset( tmpstr, 0x00, sizeof( tmpstr ));
			iStart = indexOf( input_html, "NewsContent") + 14;
			input_html = input_html + iStart; 
			iEnd=indexOf( input_html, "\",\"");
			if( iEnd < 300)
			{
			...
			strcpy( s[n].content, tmpstr );  //get content 
			strcpy( s[n].content, "..." );  //get content 

			memset( tmpstr, 0x00, sizeof( tmpstr ));
			iStart = indexOf( input_html, "NewsAddDate") + 14;
			input_html = input_html + iStart; 
			iEnd=indexOf( input_html, "\",\"");
			strncpy( tmpstr, input_html, iEnd );
			strcpy( s[n].newsreleasetime, tmpstr );  //get content 

			strcpy( s[n].articleauthor, "淘股通" );
			strcpy( s[n].sourcename, "淘股通" );
