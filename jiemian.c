...
//http://www.jiemian.com/lists/4.html 界面快讯
int jiemian( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"item-main\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"load-more\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "界面快讯" );
			strcpy( s[n].sourcename, "界面快讯" );
