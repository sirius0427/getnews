...
int news( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"newslistbox\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "%s%s", "http://news.gg-lb.com/news", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "锂电高工网" );
			strcpy( s[n].sourcename, "锂电高工网" );
