...
//http://www.ce.cn/cysc/newmain/yc/jsxw/ 中国经济网产业新闻
int cysc( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"CurrChnlCls\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.ce.cn/cysc/newmain/yc/jsxw/%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "中国经济网产经新闻" );
			strcpy( s[n].sourcename, "中国经济网产经新闻" );
...
//http://finance.ce.cn/rolling/index.shtml 中国经济网财经滚动新闻
int rolling( char *input_html, struct NewsInfo *s )
{
	char tmpstr[1024],strDate[9];
	char change[13],rate[13],secuFullcode[10],secuName[13];
	int iStart=0,iEnd;
	...
	iStart=indexOf(input_html,"\"font14\"");
	input_html = input_html + iStart;
	iEnd=indexOf(input_html,"\"JavaScript\"");
	input_html[iEnd] = '\0';
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			iEnd=indexOf( input_html, "\"");
			strncpy( tmpstr, input_html, iEnd);
			sprintf( s[n].sourceaddr, "http://finance.ce.cn/rolling/%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "中国经济网财经滚动新闻" );
			strcpy( s[n].sourcename, "中国经济网财经滚动新闻" );
