...
//http://ttticai.com/ 天天题材
int tttcai( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"cont_list");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "天天题材" );
			strcpy( s[n].sourcename, "天天题材" );
