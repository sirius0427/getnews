...
//http://news.21so.com/qqb21news/ 抢鲜报
int qqb21news( char *input_html, struct NewsInfo *s )
{
	...
	iStart=indexOf(input_html,"\"main\"");
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			strcpy( s[n].articleauthor, "抢鲜报" );
			strcpy( s[n].sourcename, "抢鲜报" );
