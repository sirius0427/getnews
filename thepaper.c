...
//http://www.thepaper.cn/channel_masonry.jsp?channelID=25951&_=1472712213908 澎湃财经
int thepaper( char *input_html, struct NewsInfo *s )
{
	...
	for( n=0;n<50; n++) //网页上一页有50条数据
	{
		...
		if( iStart != -1)
		{
			...
			sprintf( s[n].sourceaddr, "http://www.thepaper.cn/%s", tmpstr );  //sourceaddr
			...
			strcpy( s[n].articleauthor, "澎湃财经" );
			strcpy( s[n].sourcename, "澎湃财经" );
